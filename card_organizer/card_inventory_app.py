...
# --- IMPORTANT CHANGE: Cache the data reading ---
...
with tabs[1]:
    ...
    if selected_card_display != "--- Select a Card to Update ---":
        ...
        if selected_gsheet_row_index:
            try:
                row_values = inventory_ws.row_values(selected_gsheet_row_index) # This is a direct read, but typically happens less frequently
...
with tabs[2]:
    ...
    # --- Use the cached function to get data ---
    records, _ = get_inventory_data() # We only need records here
    # --- End cached data usage ---
